...
namespace Maze
{
    ...
    public class MazeActor : GameObj
    {
        ...
        public override void Update()
        {
                ...
                game.mngrLvl.opSyncActors))
            {
                    ...
                    game.IsActive)
                {
                        ...
                        foreach (GameObj item in items)
                        {
                            ...
                            if (item.IsSolid)
                            {
                                ...
                                if (item.BlockType == Type.CoinLock)
                                {
                                    ...
                                    if (game.mngrLvl.ActorCoins >= item.SlotValueInt1)
                                    {
                                        //Subtracts the coins.
                                        if (item.SlotValueInt2 == 1)
                                        {
                                            game.mngrLvl.ActorCoins -= item.SlotValueInt1;
                                        }
                                ...
                                else if (item.BlockType == Type.Crate)
                                {
                                    ...
                                    List<GameObj> itemsTop =
                                        ...
                                        Layer && o.IsEnabled && o != item).ToList();
                                    ...
                                    if (itemsFront.Count == 0 && itemsTop.Count == 0)
                                    {
                                        ...
                                        game.playlist.Play(sndMoveCrate, X, Y);
                                ...
                                {
                                    if ((item.SlotValueInt1 == 0 &&
                                        item.BlockDir != BlockDir) ||
                                        (item.SlotValueInt1 != 0 &&
                                        item.BlockDir != BlockDir &&
                                        item.BlockDir != Utils.DirOpp(BlockDir)) ||
                        ...
                        foreach (GameObj item in items)
                        {
                            ...
                            else if (item.BlockType == Type.Ice)
                            {
                                ...
                                itemsTemp = itemsTemp.Where(o =>
                                    ...
                                    && ((o.SlotValueInt1 == 0 && o.BlockDir == dirPrev) ||
                                    (o.SlotValueInt1 != 0 && (o.BlockDir == dirPrev ||
                            ...
                            {
                                if ((item.SlotValueInt1 == 0 && item.BlockDir != BlockDir) ||
                                    (item.SlotValueInt1 != 0 && item.BlockDir != BlockDir &&
                                    item.BlockDir != Utils.DirOpp(BlockDir)))
                                {
                                    canMove = false;
                                }
                    ...
                    if (doMove && canMove)
                    {
                        ...
                        foreach (GameObj item in items)
                        {
                            if (item.IsEnabled && item.SlotValueInt1 > 0)
                            {
                                item.SlotValueInt1--;
                            }
                        }
            ...
            if (IsEnabled)
            {
                Lighting.light.Intensity = game.mngrLvl.actor == this ? 0.5f : 0.1f;
            }
            #endregion
        ...
        /// <summary>
        /// Disables the actor lighting if not enabled.
        /// </summary>
        public override void UpdateLighting(bool lightDefaultVis = true, bool shadowDefaultVis = true)
        {
            base.UpdateLighting(lightDefaultVis && IsEnabled, shadowDefaultVis);
        }
