...
namespace Maze
{
    ...
    public class MazeEAuto : GameObj
    {
        ...
        {
            MazeEAuto newBlock = new(game, X, Y, Layer);
            ...
            if (IsEnabled)
            {
                ...
                if (IsActivated && ActionType > 4)
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        pickup.BlockSprite.angle = (float)(2 * Math.PI * Utils.Rng.NextDouble());
                        game.mngrLvl.AddItem(pickup);
                    }

                    //Gets all items matching the index to affect.
                    var items = game.mngrLvl.items.Where(o => o.SignalListenChannel == SignalSendChannel);
